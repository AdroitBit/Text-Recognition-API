from TextRecognizer.TextRecognizer import *
class Images:
...
class Image:
	...
	@property
	def filename(self):
		return regex.split("\\\\|/",self.name)[-1]
	@property
	def foldername(self):
		return "/".join(regex.split("\\\\|/",self.name)[0:-1])
	@property
	def folderAddress(self):
		return "/".join(regex.split("\\\\|/",self.name)[0:-1])
	...
	@staticmethod
	def fromScreen(pos1=None,pos2=None):
